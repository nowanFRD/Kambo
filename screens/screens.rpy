...
style say_dialogue:
    outlines [ (absolute(2), "#7e7e7e", absolute(0), absolute(0)) ]
...
screen navigation():
    ...
    hbox:
        ...
        if renpy.variant("pc") or (renpy.variant("web") and not renpy.variant("mobile")):
            ...
            textbutton _("Помощь") action OpenURL(
            config.gamedir + "/" + (
                "rus_help.html" if _preferences.language == None else 
...
screen clickable_up_text():
    tag ui_clickable
    zorder 100

    default col = "#000000"

    textbutton "Стреляй..." action Return(True):
        align (0.5, 0.7)
        text_color col
        hovered SetScreenVariable("col", "#ff0000")
        unhovered SetScreenVariable("col", "#000000")

screen clickable_up_text_end():
    tag ui_clickable
    zorder 100
